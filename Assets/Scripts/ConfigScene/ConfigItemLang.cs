...
/// BGM設定の処理を行うクラス
/// </summary>
public class ConfigItemLang : ConfigItemBase
{
    ...
    //--- ServerData 関連 ---
    private ServerData.LangSettings langSettings;
    ...
    private void GetServerData()
    {
        ...
        {
            langSettings = LocalStorageAPI.Instance.GetLangSettings();
        ...
        if (langSettings.IsValid())
        {
            // 選択項目を復元する。範囲外だった場合は0にして置く
            currentIndex = langSettings.langIndex;
        ...
        if (needReset)
        {
            ...
            langSettings.dataVersion = ServerData.DataVersion.LangSettings;
            langSettings.langIndex = 0;
            currentIndex = langSettings.langIndex;
        }
    }
    ...
    {
        if (langSettings.langIndex != currentIndex)
        {
            langSettings.langIndex = currentIndex;
            isDirty = true;
        }
    }
    ...
    private void SaveServerData()
    {
        if (isDirty)
        {
            ...
            if (AtsumaruAPI.Instance.IsValid())
            {
                // AtsumaruAPI.Instance.SaveSoundSettings(langSettings);
            }
            else
            {
                LocalStorageAPI.Instance.SaveLangSettings(langSettings);
            }
        }
    }
